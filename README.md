...
### ?úÍµ≠??
?ÑÎûò ?§ÌÅ¨Î¶∞ÏÉ∑?Ä ?ôÏùº??fixture Í∏∞Î∞ò Î™©ÏóÖ SQLite ?∞Ïù¥?∞Î≤†?¥Ïä§?êÏÑú ?úÍµ≠??UIÎ°??§Ïãú Ï∫°Ï≤ò???îÎ©¥?ÖÎãà?? FAKE ?òÍ≤Ω Í∞íÏ? Î°úÏª¨ UI?êÏÑú providerÍ∞Ä ?∞Í≤∞??Í≤ÉÏ≤ò???úÏãú?òÍ∏∞ ?ÑÌïú ?ºÎ≤®?¥Î©∞, ?§Ï†ú credential, provider Í≥ÑÏ†ï ?ùÎ≥Ñ?? webhook URL, Î°úÏª¨ Codex/Claude ?∏ÏÖò ?∞Ïù¥?∞Îäî ?¨Ìï®?òÏ? ?äÏäµ?àÎã§.

?Ä?úÎ≥¥??Í∞úÏöî:

![MoneySiren ?úÍµ≠??Î™©ÏóÖ ?Ä?úÎ≥¥??(docs/assets/install/moneysiren-korean-mock-dashboard.png)

CLI ?Ä?úÎ≥¥????™© ?§Ï†ï:

![MoneySiren ?úÍµ≠??CLI ?Ä?úÎ≥¥????™© ?§Ï†ï](docs/assets/install/moneysiren-korean-mock-dashboard-settings.png)

?∞Ïä§?¨ÌÜ± HUD:

![MoneySiren ?úÍµ≠??Î™©ÏóÖ HUD](docs/assets/install/moneysiren-korean-mock-hud.png)

?åÎ¶º Î∞?HUD ?§Ï†ï:

![MoneySiren ?úÍµ≠???åÎ¶º Î∞?HUD ?§Ï†ï](docs/assets/install/moneysiren-korean-mock-hud-settings.png)
