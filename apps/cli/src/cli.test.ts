...
describe("MoneySiren CLI", () => {
  ...
  it("reports an installed web runtime when the release manifest has a web asset", async () => {
    ...
    const webAssetPath = join(releaseDir, "moneysiren-web-runtime-v0.1.7-beta.5.tar.gz");
  ...
  it("prints the pinned install release default separately from the CLI package version", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(CLI_VERSION).toBe("0.1.7-beta.5");
    expect(stdout).toContain("Release default: ztwz11/moneysiren@v0.1.7-beta.5.");
  });
  ...
  it("allows unsigned Windows HUD installation only through the explicit CLI flag", async () => {
    ...
    const installDir = join(cwd, "release-install");
    const profilePath = join(cwd, "install-profile.json");
    const hudAsset = "MoneySiren.Tray_0.1.7-beta.5_x64-portable.exe";
    const hudBytes = Buffer.from("fake unsigned hud executable");

    const result = await runCli(
      ["install", "--hud", "--allow-unsigned-hud", "--dir", installDir, "--tag", "v0.1.7-beta.5"],
      {
        ...testContext(cwd, {
          MONEYSIREN_INSTALL_PROFILE_PATH: profilePath,
          MONEYSIREN_RELEASE_PLATFORM: "win32",
          ...
          if (url.endsWith("/repos/ztwz11/moneysiren/releases/tags/v0.1.7-beta.5")) {
            return Response.json({
              ...
              assets: [
                {
                  name: hudAsset,
                  browser_download_url: `https://github.com/ztwz11/moneysiren/releases/download/v0.1.7-beta.5/${hudAsset}`,
                },
                {
                  name: "moneysiren-tray-windows-SHA256SUMS.txt",
                  browser_download_url: "https://github.com/ztwz11/moneysiren/releases/download/v0.1.7-beta.5/moneysiren-tray-windows-SHA256SUMS.txt",
                },
              ],
            });
          }
