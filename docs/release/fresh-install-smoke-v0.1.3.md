...
| Strict postinstall release asset fetch | BLOCKED | `MONEYSIREN_APP_STRICT_POSTINSTALL=1 npm install -g @moneysiren/app` failed because `ztwz11/moneysiren@v0.1.5` returned `404 Not Found`. |

## Blocker

`@moneysiren/app@0.1.5` installs the CLI command successfully, but the matching GitHub Release web runtime asset was not available for `v0.1.5` during this smoke run. External users can run CLI-only commands and fixture sync, but source-free `msiren start` remains blocked until a matching GitHub Release web runtime is published.

## Next Action

Publish or repair the `v0.1.5` GitHub Release assets, or cut a new patch tag with matching web runtime assets. After that, rerun:
