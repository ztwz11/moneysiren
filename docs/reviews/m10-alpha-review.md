...
- Run `docker build --pull=false --target verify -t moneysiren:m10-verify .` in an environment with Docker available before relying on the Docker path for reviewer onboarding.
...
| `find . -path './.git' -prune -o -path './node_modules' -prune -o -path './.moneysiren' -prune -o \( \( -name '.env' -o -name '.env.*' \) ! -name '.env.example' \) -print` | PASS | No committed `.env` or `.env.*` files beyond `.env.example`. |
