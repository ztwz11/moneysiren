...
If `msiren install --web` reports `404 Not Found` for a release such as `ztwz11/moneysiren@v0.1.5`, the npm command package is installed but the matching source-free dashboard runtime asset is not available on GitHub Releases for that tag. Use the source setup in `docs/install.md` until the release asset is published.
