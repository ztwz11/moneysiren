...
if (!shouldPublish) {
  console.log([
    ...
    `Run \`npm run publish:app:latest\` to publish with tag "${npmTag}".`,
  ].join("\n"));
  process.exit(0);
}
...
run(npmBin, getPublishArgs({ dryRun: false }), {
  ...
  failureMessage: [
    ...
    "If your account has an authenticator OTP, retry with `npm run publish:app:latest -- --otp=123456` using the current code.",
